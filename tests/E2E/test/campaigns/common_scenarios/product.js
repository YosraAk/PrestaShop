...
module.exports = {
  ...
  clickOnPreviewLink(client, selector, logoSelector) {
    test('should click on the "Preview" link', () => {
      return promise
        .then(() => client.isVisible(logoSelector))
        .then(() => {
          if (global.ps_mode_dev && !global.isVisible) {
  ...
  fillOptionsTab(supplierData) {
    ...
    scenario('Create product with options in the Back Office', client => {
      ...
      test('should upload the first product picture', () => client.uploadPicture(data.virtual.picture_1, AddProductPage.picture));
      ...
      scenario('Fill "Options" form', client => {
        ...
        test('should check that the product is well displayed', async () => {
          ...
          await client.isExisting(productPage.product_image.replace('%S', (data.virtual.name).toLowerCase() + date_time), 1000);
        });
        ...
        test('should check that the product in FO with "URL"', async () => {
  ...
  chooseVisibility(client, SearchProductPage, productPage, visibilityOption, visibilityValue) {
    ...
    if (visibilityValue === 'none') {
      test('should check that the product does not exist', async () => {
        await client.clickPageNext(productPage.pagination_number_link.replace('%NUM', Math.ceil(Number(global.pagination))), 1000);
        await client.isNotExisting(productPage.product_image.replace('%S', 'pa' + date_time), 1000);
      });
      ...
      test('should check that the product does not exist', () => client.isNotExisting(productPage.product_image.replace('%S', data.virtual.name + date_time)));
      test('should search for the product', () => client.searchByValue(SearchProductPage.search_input, SearchProductPage.search_button, data.virtual.name + date_time));
